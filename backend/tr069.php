...
try {
    ...
    if (stripos($raw_post, '<cwmp:Inform>') !== false) {
        ...
        if ($serialNumber) {
            ...
            // After processing device details, always queue an info task
            try {
                // Get device_id
                ...
                if ($deviceId) {
                    ...
                    tr069_log("Always queued info task for device $serialNumber during Inform", "INFO");
                }
            } catch (PDOException $e) {
                tr069_log("Error queueing info task: " . $e->getMessage(), "ERROR");
            }
